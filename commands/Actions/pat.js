...
class Pat extends CommandStructure {
    constructor() {
        super({
            name: 'pat',
            description: 'Pat yourself/one or more users. Images are provided by weeb.sh.',
            group: 'Actions',
            aliases: ['pet'],
    ...
    async run(client, message, args) {
        ...
        ? `${getDisplayName(message.member)} pats ${mapUserGroup(users)}`
        : (users.length === 0)
        ? `${getDisplayName(message.member)} pats themselves`
        : `${getDisplayName(message.member)} pats ${users[0].user.username}`;
